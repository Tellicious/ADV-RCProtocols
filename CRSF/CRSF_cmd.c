...
//
// Spec source: tbs-fpv/tbs-crsf-spec/crsf.md — sections “0x32 Direct Commands”
// and the child pages (Command ACK, FC Commands, Bluetooth Command, OSD
// Commands, VTX Commands, LED, General, Crossfire, Flow Control Frame, Screen
// Command). This header supports the **payload part only**, i.e. bytes after
// Command_ID. Assumptions satisfied by the caller:
...
//
// Decoder:
//   crsf32_rc crsf32_decode_payload(uint8_t command_id, const uint8_t *payload,
//   size_t len,
//                                   crsf32_packet_t *out);
// Encoder:
//   int        crsf32_encode_payload(uint8_t *out, size_t cap, const
//   crsf32_packet_t *in);
//
// Notes:
// - Big-endian for multi-byte integers (per CRSF spec).
// - Strings are ASCII NUL-terminated; optional strings may be empty ("\0").
// - Where the spec marks fields as “packed” bitfields, helpers here pack them
// as
//   a 24-bit big-endian bit stream in the documented order (e.g.
//   H(9),S(7),V(8)).
...
typedef enum { // 0x32.0x0A General
  ...
  CRSF32_GEN_CRSF_PROTOCOL_SPEED_PROPOSAL =
      0x70, // uint8 port_id + uint32 proposed_baudrate
  CRSF32_GEN_CRSF_PROTOCOL_SPEED_PROPOSAL_RESPONSE =
      0x71 // uint8 port_id + bool response
...
typedef enum { // 0x32.0x20 Flow Control Frame
  CRSF32_FLOW_SUBSCRIBE =
      0x01, // uint8 Frame type + uint16 Max interval time (ms)
...
static inline void crsf32_pack_hsv24(uint16_t H, uint8_t S, uint8_t V,
                                     uint8_t out[3]) {
  uint32_t v =
      ((uint32_t)(H & 0x1FF) << 15) | ((uint32_t)(S & 0x7F) << 8) | (uint32_t)V;
  out[0] = (uint8_t)(v >> 16);
  out[1] = (uint8_t)(v >> 8);
  out[2] = (uint8_t)v;
}

static inline void crsf32_unpack_hsv24(const uint8_t in[3], uint16_t *H,
                                       uint8_t *S, uint8_t *V) {
...
// Big-endian helpers
static inline uint16_t crsf32_be16(const uint8_t *p) {
  return (uint16_t)((((uint16_t)p[0]) << 8) | p[1]);
}

static inline uint32_t crsf32_be32(const uint8_t *p) {
...
typedef struct {
  ...
  struct {
    ...
    const char
        *selectionText; // NUL-terminated; if empty, this whole struct is absent
    uint8_t value;
    uint8_t minValue;
    uint8_t maxValue;
...
static inline crsf32_rc crsf32_read_cstr(const uint8_t **q, size_t *rem,
                                         const char **out_s) {
  int z;
  if (!q || !*q || !rem || !out_s) {
...
static inline crsf32_rc crsf32_decode_payload(uint8_t command_id,
  ...
  case CRSF32_CMDID_FC: {
    ...
    out->subcommand = sub;
    switch (sub) {
    case CRSF32_FC_FORCE_DISARM:
      out->kind = CRSF32_KIND_FC_FORCE_DISARM;
      return CRSF32_OK;
  ...
  case CRSF32_CMDID_BLUETOOTH: {
    ...
    out->subcommand = sub;
    switch (sub) {
    case CRSF32_BT_RESET:
      out->kind = CRSF32_KIND_BT_RESET;
      return CRSF32_OK;
  ...
  case CRSF32_CMDID_VTX: {
    ...
    case CRSF32_VTX_SET_FREQUENCY:
      ...
      return CRSF32_OK;
    case CRSF32_VTX_CHANGE_POWER:
      out->kind = CRSF32_KIND_VTX_CHANGE_POWER;
      return CRSF32_OK;
    case CRSF32_VTX_ENABLE_PITMODE_ON_PUP:
      ...
      out->kind = CRSF32_KIND_VTX_ENABLE_PITMODE_ON_PUP;
      {
        ...
        out->v.vtx_enable_pitmode_on_pup.cfg.pitmode_control =
            (uint8_t)((b >> 5) & 0x03);
        out->v.vtx_enable_pitmode_on_pup.cfg.pitmode_switch =
            (uint8_t)((b >> 1) & 0x0F);
      }
      return CRSF32_OK;
    case CRSF32_VTX_POWER_UP_FROM_PITMODE:
      out->kind = CRSF32_KIND_VTX_POWER_UP_FROM_PITMODE;
      return CRSF32_OK;
  ...
  case CRSF32_CMDID_LED: {
    ...
    out->subcommand = sub;
    switch (sub) {
    case CRSF32_LED_SET_TO_DEFAULT:
      out->kind = CRSF32_KIND_LED_SET_TO_DEFAULT;
      return CRSF32_OK;
    ...
    case CRSF32_LED_OVERRIDE_PULSE:
      ...
      q += 2;
      crsf32_unpack_hsv24(q, &out->v.led_override_pulse.H_start,
                          &out->v.led_override_pulse.S_start,
                          &out->v.led_override_pulse.V_start);
      q += 3;
      crsf32_unpack_hsv24(q, &out->v.led_override_pulse.H_stop,
                          &out->v.led_override_pulse.S_stop,
                          &out->v.led_override_pulse.V_stop);
      return CRSF32_OK;
    case CRSF32_LED_OVERRIDE_BLINK:
      ...
      q += 2;
      crsf32_unpack_hsv24(q, &out->v.led_override_blink.H_start,
                          &out->v.led_override_blink.S_start,
                          &out->v.led_override_blink.V_start);
      q += 3;
      crsf32_unpack_hsv24(q, &out->v.led_override_blink.H_stop,
                          &out->v.led_override_blink.S_stop,
                          &out->v.led_override_blink.V_stop);
      return CRSF32_OK;
    case CRSF32_LED_OVERRIDE_SHIFT:
      ...
      q += 2;
      crsf32_unpack_hsv24(q, &out->v.led_override_shift.H,
                          &out->v.led_override_shift.S,
                          &out->v.led_override_shift.V);
      return CRSF32_OK;
    default:
      out->v.raw.bytes = q - 1;
      out->v.raw.len = rem + 1;
  ...
  case CRSF32_CMDID_CROSSFIRE: {
    ...
    out->subcommand = sub;
    switch (sub) {
    case CRSF32_CF_SET_RX_BIND_MODE:
      out->kind = CRSF32_KIND_CF_SET_RX_BIND_MODE;
      return CRSF32_OK;
    case CRSF32_CF_CANCEL_BIND_MODE:
      out->kind = CRSF32_KIND_CF_CANCEL_BIND_MODE;
      return CRSF32_OK;
    ...
    case CRSF32_CF_MODEL_SELECTION:
      ...
      return CRSF32_OK;
    case CRSF32_CF_CURRENT_MODEL_QUERY:
      out->kind = CRSF32_KIND_CF_CURRENT_MODEL_QUERY;
      return CRSF32_OK;
...
static inline int crsf32_encode_payload(uint8_t *out, size_t cap,
                                        const crsf32_packet_t *in) {
  size_t off = 0;
  if (!out || !in) {
    return CRSF32_ERR_SHORT;
  }
  ...
  case CRSF32_CMDID_FC: {
    ...
    switch (in->kind) {
    case CRSF32_KIND_FC_FORCE_DISARM:
      break;
  ...
  case CRSF32_CMDID_VTX: {
    ...
    out[off++] = in->subcommand;
    switch (in->kind) {
    case CRSF32_KIND_VTX_CHANGE_CHANNEL:
      break;
    case CRSF32_KIND_VTX_SET_FREQUENCY:
      ...
      off += 2;
      break;
    case CRSF32_KIND_VTX_CHANGE_POWER:
      break;
    case CRSF32_KIND_VTX_ENABLE_PITMODE_ON_PUP:
      ...
      {
        uint8_t b =
            (uint8_t)(((in->v.vtx_enable_pitmode_on_pup.cfg.PitMode & 1) << 7) |
                      ((in->v.vtx_enable_pitmode_on_pup.cfg.pitmode_control & 3)
                       << 5) |
                      ((in->v.vtx_enable_pitmode_on_pup.cfg.pitmode_switch &
                        0x0F)
                       << 1));
        out[off++] = b;
      }
      break;
    case CRSF32_KIND_VTX_POWER_UP_FROM_PITMODE:
      break;
    ...
    case CRSF32_KIND_VTX_SET_POWER:
      ...
      out[off++] =
          in->v.vtx_set_power.power_dBm; // NOTE: case sensitive fix not needed;
                                         // keep as in original
      break;
    default:
      break;
  ...
  case CRSF32_CMDID_LED: {
    ...
    out[off++] = in->subcommand;
    switch (in->kind) {
    case CRSF32_KIND_LED_SET_TO_DEFAULT:
      break;
    case CRSF32_KIND_LED_OVERRIDE_COLOR:
      ...
      {
        uint8_t b[3];
        crsf32_pack_hsv24(in->v.led_override_color.H,
                          in->v.led_override_color.S,
                          in->v.led_override_color.V, b);
        out[off++] = b[0];
        out[off++] = b[1];
        out[off++] = b[2];
      }
    ...
    case CRSF32_KIND_LED_OVERRIDE_PULSE:
      ...
      {
        uint8_t b[3];
        crsf32_pack_hsv24(in->v.led_override_pulse.H_start,
                          in->v.led_override_pulse.S_start,
                          in->v.led_override_pulse.V_start, b);
        out[off++] = b[0];
        out[off++] = b[1];
        out[off++] = b[2];
        crsf32_pack_hsv24(in->v.led_override_pulse.H_stop,
                          in->v.led_override_pulse.S_stop,
                          in->v.led_override_pulse.V_stop, b);
        out[off++] = b[0];
        out[off++] = b[1];
        out[off++] = b[2];
      }
    ...
    case CRSF32_KIND_LED_OVERRIDE_BLINK:
      ...
      {
        uint8_t b[3];
        crsf32_pack_hsv24(in->v.led_override_blink.H_start,
                          in->v.led_override_blink.S_start,
                          in->v.led_override_blink.V_start, b);
        out[off++] = b[0];
        out[off++] = b[1];
        out[off++] = b[2];
        crsf32_pack_hsv24(in->v.led_override_blink.H_stop,
                          in->v.led_override_blink.S_stop,
                          in->v.led_override_blink.V_stop, b);
        out[off++] = b[0];
        out[off++] = b[1];
        out[off++] = b[2];
      }
    ...
    case CRSF32_KIND_LED_OVERRIDE_SHIFT:
      ...
      {
        uint8_t b[3];
        crsf32_pack_hsv24(in->v.led_override_shift.H,
                          in->v.led_override_shift.S,
                          in->v.led_override_shift.V, b);
        out[off++] = b[0];
        out[off++] = b[1];
        out[off++] = b[2];
      }
  ...
  case CRSF32_CMDID_GENERAL: {
    ...
    case CRSF32_KIND_GEN_PROTOCOL_SPEED_PROPOSAL:
      ...
      crsf32_wbe32(out + off,
                   in->v.gen_protocol_speed_proposal.proposed_baudrate);
      off += 4;
      break;
    case CRSF32_KIND_GEN_PROTOCOL_SPEED_RESPONSE:
      ...
      out[off++] =
          (uint8_t)(in->v.gen_protocol_speed_response.response ? 1 : 0);
      break;
    default:
      break;
    }
  ...
  case CRSF32_CMDID_CROSSFIRE: {
    ...
    case CRSF32_KIND_CF_CURRENT_MODEL_QUERY:
      break;
  ...
  case CRSF32_CMDID_SCREEN: {
    ...
    case CRSF32_KIND_SCREEN_POPUP_MESSAGE_START: {
      const char *h = in->v.screen_popup_message_start.Header
                          ? in->v.screen_popup_message_start.Header
                          : "";
      const char *im = in->v.screen_popup_message_start.Info_message
                           ? in->v.screen_popup_message_start.Info_message
                           : "";
      const char *s;
      for (s = h; *s; ++s) {
        if (off >= cap) {
      ...
      out[off++] =
          (uint8_t)(in->v.screen_popup_message_start.Close_button_option ? 1
                                                                         : 0);
      if (in->v.screen_popup_message_start.add_data.present) {
        const char *sel =
            in->v.screen_popup_message_start.add_data.selectionText
                ? in->v.screen_popup_message_start.add_data.selectionText
                : "";
        for (s = sel; *s; ++s) {
          if (off >= cap) {
        ...
        {
          const char *unit =
              in->v.screen_popup_message_start.add_data.unit
                  ? in->v.screen_popup_message_start.add_data.unit
                  : "";
          for (s = unit; *s; ++s) {
            if (off >= cap) {
      ...
      if (in->v.screen_popup_message_start.has_possible_values) {
        const char *pv = in->v.screen_popup_message_start.possible_values
                             ? in->v.screen_popup_message_start.possible_values
                             : "";
        for (s = pv; *s; ++s) {
          if (off >= cap) {
            return CRSF32_ERR_SHORT;
