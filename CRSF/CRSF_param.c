// crsf_param_2b.h — single-header encoder/decoder for CRSF 0x2B "Parameter
// Settings (Entry)" frames
//
// Usage:
...
// - FLOAT is fixed-point: int32 values with `precision` decimal places and
// `step`.
// - FOLDER may include an optional child-index list terminated by 0xFF. Decoder
// accepts both.
...
static inline uint16_t crsf_be16(const uint8_t *p) {
  return (uint16_t)((p[0] << 8) | p[1]);
}

static inline uint32_t crsf_be32(const uint8_t *p) {
  return ((uint32_t)p[0] << 24) | ((uint32_t)p[1] << 16) |
         ((uint32_t)p[2] << 8) | p[3];
}

static inline uint64_t crsf_be64(const uint8_t *p) {
  return ((uint64_t)crsf_be32(p) << 32) | crsf_be32(p + 4);
}
...
// ---- Decoder -------------------------------------------------------------
// Decodes a 0x2B entry payload that starts at [parent]. On success, returns
// CRSF_P2B_OK and fills *out. Pointers in *out refer to 'payload' memory; keep
// it alive while using the struct.

static inline crsf_p2b_rc
crsf_param2b_decode_from_parent(const uint8_t *payload, size_t len,
  ...
  case CRSF_PARAM_T_UINT64: {
    size_t w =
        (out->type == CRSF_PARAM_T_INT8 || out->type == CRSF_PARAM_T_UINT8) ? 1
        : (out->type == CRSF_PARAM_T_INT16 || out->type == CRSF_PARAM_T_UINT16)
            ? 2
        : (out->type == CRSF_PARAM_T_INT32 || out->type == CRSF_PARAM_T_UINT32)
            ? 4
            : 8;
    ...
    bool is_signed =
        (out->type == CRSF_PARAM_T_INT8 || out->type == CRSF_PARAM_T_INT16 ||
         out->type == CRSF_PARAM_T_INT32 || out->type == CRSF_PARAM_T_INT64);
    switch (w) {
    case 1:
    ...
    case 8:
      ...
      max =
          is_signed ? (int64_t)crsf_be64(p + 16) : (uint64_t)crsf_be64(p + 16);
      break;
    }
    out->v.i.cur = cur;
    out->v.i.min = min;
...
// ---- Encoder -------------------------------------------------------------
// Encodes a 0x2B entry payload that starts at [parent] into outb (capacity
// out_cap). Returns number of bytes written, or a negative error code.

static inline int
crsf_param2b_encode_from_parent(uint8_t *outb, size_t out_cap,
                                const crsf_param_entry_t *in) {
  size_t off = 0;
  if (out_cap < 2) {
  ...
  case CRSF_PARAM_T_UINT64: {
    size_t w =
        (in->type == CRSF_PARAM_T_INT8 || in->type == CRSF_PARAM_T_UINT8) ? 1
        : (in->type == CRSF_PARAM_T_INT16 || in->type == CRSF_PARAM_T_UINT16)
            ? 2
        : (in->type == CRSF_PARAM_T_INT32 || in->type == CRSF_PARAM_T_UINT32)
            ? 4
            : 8;
    if (off + w * 3 > out_cap) {
      return CRSF_P2B_ERR_SHORT;
