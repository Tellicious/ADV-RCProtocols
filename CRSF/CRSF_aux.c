...
typedef enum {
  FC_CMD_FORCE_DISARM = 0x01,
  FC_CMD_SCALE_CHANNEL = 0x02
} fc_command_t;
...
typedef enum {
  GENERAL_CMD_PROTOCOL_SPEED_PROPOSAL = 0x70,
  GENERAL_CMD_PROTOCOL_SPEED_RESPONSE = 0x71
} general_command_t;
...
typedef enum {
  FLOW_CMD_SUBSCRIBE = 0x01,
  FLOW_CMD_UNSUBSCRIBE = 0x02
} flow_control_command_t;

// Screen Commands (0x32.0x22)
typedef enum {
  SCREEN_CMD_POPUP_START = 0x01,
  SCREEN_CMD_SELECTION_RETURN = 0x02
} screen_command_t;
...
typedef struct {
  // First byte determines if this struct exists - if it's null, only 1 byte
  // total
  char selection_text[32]; // Null-terminated string, max practical size //TODO
                           // check max length
  uint8_t value;
  uint8_t min_value;
  uint8_t max_value;
  uint8_t default_value;
  char unit[16]; // Null-terminated string of measurement unit //TODO check max
                 // length
...
typedef struct {
  ...
  uint8_t
      close_button_option; // bool: 1 = show close button, 0 = no close button
  crsf_popup_add_data_t
      add_data; // Optional additional data (check if selection_text[0] != 0)
  char possible_values[128]; // Optional semicolon-separated values,
                             // null-terminated
...
uint8_t crsf_get_expected_length(uint8_t subcommand, uint8_t sub_subcommand) {
  ...
  case CRSF_SUBCMD_FC_COMMANDS:
    return 2; // 1 subcmd + 1 fc_command
  ...
  case CRSF_SUBCMD_CROSSFIRE:
    ...
    case CF_CMD_REPLY_MODEL_SELECTION:
      return 3;
    default:
      return 2; // Most are bare commands
    }
...
uint8_t parse_string(const uint8_t *data, uint8_t start_offset, char *dest,
                     uint8_t max_len) {
...
void crsf_unpack_0x32_payload_auto(const uint8_t *payload,
                                   void *output_struct) {
  ...
  case CRSF_SUBCMD_BLUETOOTH: {
    ...
    if (bt_cmd == BT_CMD_ENABLE) {
      crsf_bluetooth_enable_t *bt_enable =
          (crsf_bluetooth_enable_t *)output_struct;
      bt_enable->bt_command = bt_cmd;
      bt_enable->enable = data[1];
    } else {
      crsf_bluetooth_command_t *bt_basic =
          (crsf_bluetooth_command_t *)output_struct;
  ...
  case CRSF_SUBCMD_VTX: {
    ...
    switch (vtx_cmd) {
    case VTX_CMD_CHANGE_CHANNEL: {
      crsf_vtx_change_channel_t *vtx_ch =
          (crsf_vtx_change_channel_t *)output_struct;
      vtx_ch->vtx_command = vtx_cmd;
      vtx_ch->channel = data[1];
      break;
    }
    case VTX_CMD_SET_FREQUENCY: {
      crsf_vtx_set_frequency_t *vtx_freq =
          (crsf_vtx_set_frequency_t *)output_struct;
    ...
    case VTX_CMD_POWERUP_FROM_PITMODE: {
      crsf_vtx_powerup_pitmode_t *vtx_powerup =
          (crsf_vtx_powerup_pitmode_t *)output_struct;
      vtx_powerup->vtx_command = vtx_cmd;
      break;
    }
    case VTX_CMD_SET_DYNAMIC_POWER: {
      crsf_vtx_dynamic_power_t *vtx_dyn =
          (crsf_vtx_dynamic_power_t *)output_struct;
  ...
  case CRSF_SUBCMD_GENERAL: {
    ...
    switch (gen_cmd) {
    case GENERAL_CMD_PROTOCOL_SPEED_PROPOSAL: {
      crsf_protocol_speed_proposal_t *speed_prop =
          (crsf_protocol_speed_proposal_t *)output_struct;
      speed_prop->general_command = gen_cmd;
      speed_prop->port_id = data[1];
      speed_prop->proposed_baudrate =
          (data[5] << 24) | (data[4] << 16) | (data[3] << 8) | data[2];
      break;
    }
    case GENERAL_CMD_PROTOCOL_SPEED_RESPONSE: {
      crsf_protocol_speed_response_t *speed_resp =
          (crsf_protocol_speed_response_t *)output_struct;
  ...
  case CRSF_SUBCMD_CROSSFIRE: {
    ...
    switch (cf_cmd) {
    case CF_CMD_MODEL_SELECTION: {
      crsf_model_selection_t *model_sel =
          (crsf_model_selection_t *)output_struct;
      model_sel->cf_command = cf_cmd;
      model_sel->model_number = data[1];
      break;
    }
    case CF_CMD_REPLY_MODEL_SELECTION: {
      crsf_model_selection_reply_t *model_reply =
          (crsf_model_selection_reply_t *)output_struct;
    ...
    default: {
      ...
      crsf_crossfire_command_t *cf_basic =
          (crsf_crossfire_command_t *)output_struct;
  ...
  case CRSF_SUBCMD_FLOW_CONTROL: {
    ...
    case FLOW_CMD_UNSUBSCRIBE: {
      crsf_flow_unsubscribe_t *flow_unsub =
          (crsf_flow_unsubscribe_t *)output_struct;
  ...
  case CRSF_SUBCMD_SCREEN: {
    ...
    case SCREEN_CMD_POPUP_START: {
      crsf_screen_popup_start_t *popup =
          (crsf_screen_popup_start_t *)output_struct;
      ...
      offset = parse_string(data, offset, popup->info_message,
                            sizeof(popup->info_message));
      ...
      offset = parse_string(data, offset, popup->add_data.selection_text,
                            sizeof(popup->add_data.selection_text));
      ...
      if (popup->add_data.selection_text[0] != '\0') {
        ...
        // Parse unit string
        offset = parse_string(data, offset, popup->add_data.unit,
                              sizeof(popup->add_data.unit));
      }

      // Parse possible values string (auto-detect end)
      parse_string(data, offset, popup->possible_values,
                   sizeof(popup->possible_values));
      break;
    }
    case SCREEN_CMD_SELECTION_RETURN: {
      crsf_screen_selection_return_t *sel_ret =
          (crsf_screen_selection_return_t *)output_struct;
...
void crsf_unpack_0x32_payload(const uint8_t *payload, uint8_t payload_length,
                              void *output_struct) {
  ...
  case CRSF_SUBCMD_FLOW_CONTROL: {
    if (data_length >= 1) {
      ...
      case FLOW_CMD_SUBSCRIBE: {
        if (data_length >= 4) {
          crsf_flow_subscribe_t *flow_sub =
              (crsf_flow_subscribe_t *)output_struct;
          flow_sub->flow_command = flow_cmd;
          flow_sub->frame_type = data[1];
          flow_sub->max_interval_ms = (data[3] << 8) | data[2];
        }
  ...
  case CRSF_SUBCMD_SCREEN: {
    if (data_length >= 1) {
      ...
      case SCREEN_CMD_POPUP_START: {
        ...
        // Parse header string
        uint8_t header_len =
            strnlen((const char *)&data[offset], sizeof(popup->header) - 1);
        ...
        if (offset < data_length) {
          uint8_t info_len = strnlen((const char *)&data[offset],
                                     sizeof(popup->info_message) - 1);
        ...
        if (offset < data_length) {
          uint8_t sel_len = strnlen((const char *)&data[offset],
                                    sizeof(popup->add_data.selection_text) - 1);
          strncpy(popup->add_data.selection_text, (const char *)&data[offset],
                  sel_len);
          ...
          if (sel_len > 0 && offset + 4 < data_length) {
            ...
            // Parse unit string
            if (offset < data_length) {
              uint8_t unit_len = strnlen((const char *)&data[offset],
                                         sizeof(popup->add_data.unit) - 1);
              strncpy(popup->add_data.unit, (const char *)&data[offset],
                      unit_len);
        ...
        if (offset < data_length) {
          uint8_t pv_len = strnlen((const char *)&data[offset],
                                   sizeof(popup->possible_values) - 1);
